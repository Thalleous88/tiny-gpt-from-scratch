...
def softmax_overflow_demo(large_value):
    ...
    res = np.exp(large_value)
    r = {}

    r['naive_exp'] = res
    r['overflowed'] = np.isinf(res)
...
def stable_softmax_2d_rowwise(logits):
    ...
    logits = [logits[i] - np.max(logits[i]) for i in range(len(logits))]
    
    return np.array([np.exp(logits[i])/np.sum(np.exp(logits[i])) for i in range(len(logits))])
...
def sample_random_batch_offsets(data_len, block_size, batch_size, rng):
    ...
    res = rng.integers(low=0, high=high, size=batch_size)
    return res
...
def stack_x_batch(data, offsets, block_size):
    ...
    res = np.array([data[o:o+block_size] for o in offsets])
...
def stack_y_batch(data, offsets, block_size):
    ...
    res = np.array([data[o+1:o+1+block_size] for o in offsets])
...
def normalize_counts_to_probs(n_matrix):
    ...
    div = row_sums_of_counts(n_matrix)
...
def sample_next_token(p_matrix, current_id, rng):
    ...
    return rng.choice(len(p_matrix[0]), p=p_matrix[current_id])
...
def generate_sequence(p_matrix, start_id, length, rng):
    ...
    while length-1 > 0:
        next_id = sample_next_token(p_matrix, i, rng)
        res.append(next_id)
        length -= 1
        i = next_id
...
def sum_negative_log_probs(p_matrix, data):
    ...
    for i in range(len(data) - 1):
        res += log_prob_of_pair(p_matrix, data[i], data[i+1])
...
def observe_lookup_equivalence(w, ids):
    ...
    out = {}
    out['onehot_result'] = np.eye(w.shape[0])[ids] @ w

    out['index_result'] = w[ids]
